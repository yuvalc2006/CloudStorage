...
CFileHandler::CFileHandler(const std::string& filename) : filename(filename), _fileStream(nullptr), _open(false) {}
...
// Open File for Read/Write and Create Directories if Needed
size_t CFileHandler::open(const std::string& filepath, bool write) {
    if (_open) close();  // Ensure any open stream is closed first.

    std::ios_base::openmode mode = std::ios::in;
    if (write) mode = std::ios::out | std::ios::trunc;

    _fileStream.reset();
    _fileStream = std::make_unique<std::fstream>(filepath, write ? std::ios::out : std::ios::in);
...
bool CFileHandler::read(uint8_t* const dest, const size_t bytes) const {
    if (!_fileStream || !_open || !dest || bytes == 0) return false;
    try {
...
bool CFileHandler::write(const uint8_t* const src, const size_t bytes) const {
    if (!_fileStream || !_open || !src || bytes == 0) return false;
    try {
...
bool CFileHandler::readLine(std::string& line) const {
    if (!_fileStream || !_open) return false;
    try {
...
// Write a Line to File Stream
bool CFileHandler::writeLine(const std::string& line) const {
    std::string lineWithNewline = line + '\n';
    return write(reinterpret_cast<const uint8_t*>(lineWithNewline.c_str()), lineWithNewline.size());
}
...
size_t CFileHandler::size() const {
    if (!_fileStream || !_open) return 0;
    try {
        auto cur = _fileStream->tellg();
        _fileStream->seekg(0, std::fstream::end);
        auto size = _fileStream->tellg();
        _fileStream->seekg(cur); // Restore original position
...
unsigned long CFileHandler::computeCRC() {
    if (!_open) return 0;
    std::vector<char> buffer(size());
    read(reinterpret_cast<uint8_t*>(buffer.data()), buffer.size());
    return memcrc(buffer.data(), buffer.size());
}

// CRC Calculation for Memory Buffer
unsigned long CFileHandler::memcrc(const char* buffer, size_t n) {
    unsigned long s = 0;
    for (size_t i = 0; i < n; ++i) {
        unsigned int tabidx = (s >> 24) ^ static_cast<unsigned char>(buffer[i]);
        s = (s << 8) ^ crctab[0][tabidx];
    }
    while (n) {
        unsigned int c = n & 0xFF;
        n >>= 8;
        s = (s << 8) ^ crctab[0][(s >> 24) ^ c];
    }
    return ~s;
}
...
bool CFileHandler::writeAtOnce(const std::string& filepath, const std::string& data) {
    if (data.empty() || !open(filepath, true)) return false;
    bool success = write(reinterpret_cast<const uint8_t*>(data.c_str()), data.size());
    close();
    return success;
}
